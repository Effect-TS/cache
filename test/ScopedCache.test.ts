import * as ScopedCache from "@effect/cache/ScopedCache"
...
import * as WatchableLookup from "@effect/cache/test/utils/WatchableLookup"
...
describe.concurrent("ScopedCache", () => {
            ...
            Effect.flatMap((cache) =>
              pipe(
                Effect.forEachParDiscard(
                  ...
                  (n) => Effect.scoped(Effect.zipRight(ScopedCache.get(cache, n), Effect.unit()))
                ),
                Effect.flatMap(() => ScopedCache.cacheStats(cache))
              )
            )
          ))
          expect(hits).toBe(49)
  ...
  it.effect("invalidate - should properly remove and clean a resource from the cache", () =>
    Effect.gen(function*($) {
      ...
      const scopedCache = ScopedCache.make(
        capacity,
        Duration.infinity,
        (key: number) => Chunk.unsafeGet(observablesResources, key).scoped
      )
      yield* $(Effect.scoped(Effect.gen(function*($) {
        ...
        yield* $(ScopedCache.invalidate(cache, 42))
        const cacheContainsKey42 = yield* $(ScopedCache.contains(cache, 42))
        const { hits, misses, size } = yield* $(ScopedCache.cacheStats(cache))
        yield* $(Chunk.unsafeGet(observablesResources, 42).assertAcquiredOnceAndCleaned())
  ...
  it.effect("invalidate - should not invalidate anything before effect is evaluated", () =>
    Effect.gen(function*($) {
      ...
      yield* $(Effect.scoped(Effect.gen(function*($) {
        const cache = yield* $(scopedCache)
        yield* $(Effect.scoped(Effect.zipRight(ScopedCache.get(cache, void 0), Effect.unit())))
        const invalidateEffect = ScopedCache.invalidate(cache, void 0)
        const cacheContainsKey42BeforeInvalidate = yield* $(ScopedCache.contains(cache, void 0))
        yield* $(observablesResource.assertAcquiredOnceAndNotCleaned())
        yield* $(Effect.scoped(Effect.zipRight(ScopedCache.get(cache, void 0), Effect.unit())))
        yield* $(invalidateEffect)
        const cacheContainsKey42AfterInvalidate = yield* $(ScopedCache.contains(cache, void 0))
    ...
    Effect.gen(function*($) {
      ...
      yield* $(Effect.scoped(Effect.gen(function*($) {
        ...
        const contains = yield* $(pipe(
          Effect.forEachPar(
            Chunk.range(0, capacity - 1),
            (n) => Effect.scoped(ScopedCache.contains(cache, n))
          ),
          Effect.map(Chunk.every(identity))
        ))
        const { hits, misses, size } = yield* $(ScopedCache.cacheStats(cache))
        yield* $(Effect.forEach(
          observablesResources,
  ...
  it.effect("get - should not put anything in the cache before the scoped effect returned by get is used", () =>
    Effect.gen(function*($) {
      ...
      yield* $(Effect.scoped(Effect.gen(function*($) {
        ...
        ScopedCache.get(cache, void 0)
        yield* $(observablesResource.assertNotAcquired())
        const contains = yield* $(ScopedCache.contains(cache, void 0))
        expect(contains).toBe(false)
      })))
    }))
  ...
  it.effect("get - sequential use of the scoped effect returned by a single call to get should create only one resource", () =>
    Effect.gen(function*($) {
      ...
      yield* $(Effect.scoped(Effect.gen(function*($) {
        ...
        const resourceScopedProxy = ScopedCache.get(cache, void 0)
        yield* $(subResource.assertNotAcquired())
  ...
  it.effect("get - sequential use should create only one resource", () =>
    Effect.gen(function*($) {
      ...
      yield* $(Effect.scoped(Effect.gen(function*($) {
        const cache = yield* $(scopedCache)
        yield* $(subResource.assertNotAcquired())
        yield* $(Effect.scoped(ScopedCache.get(cache, void 0)))
        yield* $(subResource.assertAcquiredOnceAndNotCleaned())
        yield* $(Effect.scoped(ScopedCache.get(cache, void 0)))
        yield* $(subResource.assertAcquiredOnceAndNotCleaned())
      })))
  ...
  it.effect("get - concurrent use of the scoped effect returned by a single call to get should create only one resource", () =>
    Effect.gen(function*($) {
      ...
      yield* $(Effect.scoped(Effect.gen(function*($) {
        const cache = yield* $(scopedCache)
        const scoped = ScopedCache.get(cache, void 0)
        const scope1 = yield* $(Scope.make())
        const scope2 = yield* $(Scope.make())
        const acquire1 = Effect.provideContext(scoped, Context.make(Scope.Tag, scope1))
        const release1: Scope.Scope.Finalizer = (exit) => Scope.close(scope1, exit)
        const acquire2 = Effect.provideContext(scoped, Context.make(Scope.Tag, scope2))
  ...
  it.effect("get - when two scoped effects obtained by a single scoped effect returned by a single call to get live longer than the cache, the resource should be cleaned only when it is not in use anymore", () =>
    Effect.gen(function*($) {
      ...
      const [release1, release2] = yield* $(Effect.scoped(Effect.gen(function*($) {
        const cache = yield* $(scopedCache)
        const scoped = ScopedCache.get(cache, void 0)
        yield* $(Effect.provideContext(scoped, Context.make(Scope.Tag, scope1)))
        yield* $(Effect.provideContext(scoped, Context.make(Scope.Tag, scope2)))
        const release1: Scope.Scope.Finalizer = (exit) => Scope.close(scope1, exit)
        const release2: Scope.Scope.Finalizer = (exit) => Scope.close(scope2, exit)
        return [release1, release2] as const
      })))
        ...
        yield* $(Effect.scoped(Effect.gen(function*($) {
          ...
          yield* $(
            Effect.forEachDiscard(
              ...
              (key) => Effect.scoped(Effect.asUnit(ScopedCache.get(cache, key)))
            )
          )
          ...
          yield* $(Effect.collectAllDiscard(oldestResourceCleaned))
          ...
          yield* $(Effect.collectAllDiscard(newestResourceNotCleanedYet))
        })))
      })
      return Effect.runPromise(program)
    }))
  ...
  it.effect("get - should recall lookup function if resource is too old and release old resource when using the scoped effect multiple times", () =>
    Effect.gen(function*($) {
      ...
      yield* $(Effect.scoped(Effect.gen(function*($) {
        const cache = yield* $(ScopedCache.make(10, Duration.seconds(10), (key: void) => watchableLookup(key)))
        const scoped = Effect.scoped(Effect.asUnit(ScopedCache.get(cache, void 0)))
        yield* $(scoped)
        yield* $(TestClock.adjust(Duration.seconds(5)))
        yield* $(scoped)
        yield* $(watchableLookup.assertCalledTimes(void 0, (n) => expect(n).toBe(1)))
  ...
  it.effect("get - when resource is expired but still used it should wait until resource is not cleaned anymore to clean immediately", () =>
    Effect.gen(function*($) {
      ...
      yield* $(Effect.scoped(Effect.gen(function*($) {
        ...
        yield* $(Effect.scoped(Effect.asUnit(ScopedCache.get(cache, void 0))))
        yield* $(watchableLookup.assertCalledTimes(void 0, (n) => expect(n).toBe(2)))
  ...
  it.effect("refresh - should update the cache with a new value", () =>
    Effect.gen(function*($) {
      ...
      const [val1, val2, val3] = yield* $(Effect.scoped(Effect.gen(function*($) {
        const cache = yield* $(scopedCache)
        const val1 = yield* $(ScopedCache.get(cache, key))
        yield* $(ScopedCache.refresh(cache, key))
        const val2 = yield* $(ScopedCache.get(cache, key))
        const val3 = yield* $(ScopedCache.get(cache, key))
  ...
  it.effect("refresh - should clean old resource when making a new one", () =>
    Effect.gen(function*($) {
      ...
      yield* $(Effect.scoped(Effect.gen(function*($) {
        const cache = yield* $(scopedCache)
        yield* $(Effect.scoped(ScopedCache.get(cache, void 0)))
        yield* $(ScopedCache.refresh(cache, void 0))
  ...
  it.effect("refresh - should update the cache with a new value even if the last get or refresh failed", () =>
    Effect.gen(function*($) {
      ...
      const result = yield* $(Effect.scoped(Effect.gen(function*($) {
        const cache = yield* $(scopedCache)
        const failure1 = yield* $(Effect.either(ScopedCache.get(cache, key)))
        yield* $(ScopedCache.refresh(cache, key))
        const value1 = yield* $(Effect.either(ScopedCache.get(cache, key)))
        yield* $(ScopedCache.refresh(cache, key))
        const failure2 = yield* $(Effect.either(ScopedCache.refresh(cache, key)))
        yield* $(ScopedCache.refresh(cache, key))
        const value2 = yield* $(Effect.either(ScopedCache.get(cache, key)))
  ...
  it.effect("refresh - should create and acquire subresource if the key doesn't exist in the cache", () =>
    Effect.gen(function*($) {
      ...
      yield* $(Effect.scoped(Effect.gen(function*($) {
        const cache = yield* $(scopedCache)
        const count0 = yield* $(ScopedCache.size(cache))
        yield* $(Effect.forEachDiscard(Chunk.range(1, capacity), (key) => ScopedCache.refresh(cache, key)))
        const count1 = yield* $(ScopedCache.size(cache))
      ...
      const program = Effect.gen(function*($) {
        ...
        yield* $(Effect.scoped(Effect.gen(function*($) {
          ...
          yield* $(Effect.forEachDiscard(
            Chunk.range(0, numCreatedKey - 1),
            (key) => ScopedCache.refresh(cache, key)
          ))
          const createdResources = yield* $(watchableLookup.createdResources())
          const cleanedAssertions = numCreatedKey - cacheSize - 1
          const oldestResourceCleaned = cleanedAssertions <= 0
  ...
  it.effect("refresh - should not clean the resource if it's not yet expired until the new resource is ready", () =>
    Effect.gen(function*($) {
      ...
      yield* $(Effect.scoped(Effect.gen(function*($) {
        ...
        const refreshFiber = yield* $(Effect.fork(ScopedCache.refresh(cache, void 0)))
        yield* $(pipe(
